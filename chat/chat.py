...
class Chat:
    ...
    async def new_chat(self):
        ...
        if f"{self.message_type}-{self.chat_user_id}-{self.character_name}.jsonl" not in files_list:
            with open(os.path.join(script_dir, relavative_path,f"{self.message_type}-{self.chat_user_id}-{self.character_name}.jsonl"), 'w', encoding='utf-8') as f:
                print("创建新聊天")
    ...
    #保存聊天记录
    async def save_chat_message(self,assistant_reply):
        files_list = []
        relavative_path = ""
        if self.message_type == "group":
            files_list = os.listdir(group_chatmessage_path)
            ...
            files_list = os.listdir(private_chatmessage_path)
            relavative_path = "../data/chat_data/private_chat"
        if f"{self.message_type}-{self.chat_user_id}-{self.character_name}.jsonl" in files_list:
            with open(os.path.join(script_dir, relavative_path, f"{self.message_type}-{self.chat_user_id}-{self.character_name}.jsonl"),
                      'a+',encoding='utf-8') as f:
                chat_note = {}
                now = datetime.now()
                formatted_now = now.strftime("%Y-%m-%d@%H:%M:%S")
                chat_note["name"]=self.nickname
                chat_note["is_user"]=True
                chat_note["user_id"]=self.user_id
                chat_note["is_system"]=False
                chat_note["msg"]=self.message
                chat_note["create_date"]=formatted_now
                f.write(json.dumps(chat_note,ensure_ascii=False)+'\n',)

                chat_note["name"]=self.character_name
    ...
    async def get_context(self):
        ...
        if self.message_type == "group":
            ...
            relavative_path = "../data/chat_data/private_chat"
        if f"{self.message_type}-{self.chat_user_id}-{self.character_name}.jsonl" in files_list:
            file_path=os.path.join(script_dir,relavative_path,f"{self.message_type}-{self.chat_user_id}-{self.character_name}.jsonl")
